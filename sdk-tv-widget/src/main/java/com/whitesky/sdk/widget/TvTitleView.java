...
import com.whitesky.sdk.utils.TitleViewUtil;

import java.util.Timer;

public class TvTitleView extends RelativeLayout
{
    ...
    public void initTitleView()
    {
        ...
        typeface = Typeface.createFromAsset(context.getAssets(), "font/helvetica_neueltpro_thex.otf");
        ...
        context.getApplicationContext().registerReceiver(wifiChange, new IntentFilter(WifiManager.RSSI_CHANGED_ACTION));
    }
    ...
    private BroadcastReceiver wifiChange = new BroadcastReceiver()
    {
        ...
        public void onReceive(Context context, Intent intent)
        {
            ...
            if (wifiInfo.getBSSID() != null)
            {
                ...
                if (signalLevel == 0)
                {
                    imgNetWorkState.setImageDrawable(context.getResources().getDrawable(R.drawable.wifi_1));
                    
                }
                else if (signalLevel == 1)
                {
                    imgNetWorkState.setImageDrawable(context.getResources().getDrawable(R.drawable.wifi_2));
                    
                }
                else if (signalLevel == 2)
                {
                    imgNetWorkState.setImageDrawable(context.getResources().getDrawable(R.drawable.wifi_3));
                    
                }
                else if (signalLevel == 3)
                {
                    imgNetWorkState.setImageDrawable(context.getResources().getDrawable(R.drawable.networkstate_on));
                }
                if (d)
    ...
    private BroadcastReceiver mConnReceiver = new BroadcastReceiver()
    {
        public void onReceive(Context context, Intent intent)
        {
                ...
                && currentNetworkInfo.getType() == ConnectivityManager.TYPE_ETHERNET)
            {
                ...
                imgNetWorkState.setImageDrawable(context.getResources().getDrawable(R.drawable.networkstate_ethernet));
                
            }
            else if (!currentNetworkInfo.isConnected())
            {
                imgNetWorkState.setImageDrawable(context.getResources().getDrawable(R.drawable.networkstate_off));
            }
            
            if (d)
                Toast.makeText(context,
                    "currentNetworkInfo.getType()=>>" + currentNetworkInfo.getType() + currentNetworkInfo.getTypeName(),
                    Toast.LENGTH_LONG)
                    .show();
        }
    };
