...
public class LabelView extends View
{
    ...
    private void obtainAttributes(Context context, AttributeSet attrs)
    {
        ...
        float dd = mFillTriangle ? dp2px(35) : dp2px(50);
        mMinSize = ta.getDimension(R.styleable.LabelView_lv_min_size, dd);
        ...
        mRadius = ta.getDimensionPixelOffset(R.styleable.LabelView_lv_radius, 0);
        ta.recycle();
    }
    ...
    /**
     * Gravity.TOP | Gravity.LEFT Gravity.TOP | Gravity.RIGHT Gravity.BOTTOM | Gravity.LEFT Gravity.BOTTOM |
     * Gravity.RIGHT
     */
    ...
    private void drawText(int size, float degrees, Canvas canvas, float textHeight, boolean isTop)
    {
        ...
        {
            canvas.drawText(mTextAllCaps ? mTextContent.toUpperCase() : mTextContent, getPaddingLeft()
                + (size - getPaddingLeft() - getPaddingRight()) / 2, textBaseY, mTextPaint);
        }
        canvas.restore();
    }
    ...
    private void drawTextWhenFill(int size, float degrees, Canvas canvas, boolean isTop)
    {
        canvas.save();
        canvas.rotate(degrees, size / 2f, size / 2f);
        float delta = isTop ? -size / 5 : size / 5;
        float textBaseY = size / 2 - (mTextPaint.descent() + mTextPaint.ascent()) / 2 + delta;
        canvas.drawText(mTextAllCaps ? mTextContent.toUpperCase() : mTextContent, getPaddingLeft()
            + (size - getPaddingLeft() - getPaddingRight()) / 2, textBaseY, mTextPaint);
        canvas.restore();
    ...
    private int measureWidth(int widthMeasureSpec)
    {
        ...
        else
        {
            ...
            result = Math.max(result, (int)textWidth);
