                                ...
                                absListView.addOnLayoutChangeListener(new View.OnLayoutChangeListener()
                                {
                                        ...
                                        int oldLeft, int oldTop, int oldRight, int oldBottom)
                                    {
                                        try
                                        {
                                            ...
                                            {
                                                factorX =
                                                    (Math.abs(rect.left - rect.right) - firstView.getMeasuredWidth()) / 2 - 1;
                                                factorY =
                                                    (Math.abs(rect.top - rect.bottom) - firstView.getMeasuredHeight()) / 2;
                                            }
        ...
        try
        {
            ...
            scaleAnimator.addUpdateListener(new ValueAnimator.AnimatorUpdateListener()
            {
                ...
                public synchronized void onAnimationUpdate(ValueAnimator animation)
                {
                    ...
                    // Log.d(TAG,"width:"+width+" height:"+height+" translationX:"+translationX+"
            ...
            if (oldFocus != null && newFocus != null)
            {
                if (oldFocus.getParent() != newFocus.getParent())
                {
                    ...
                    if ((attacheViews.indexOf(newFocus.getParent()) < 0)
                        || (attacheViews.indexOf(oldFocus.getParent()) < 0 && attacheViews.indexOf(newFocus.getParent()) > 0))
                    {
                        mTarget.setVisibility(View.INVISIBLE);
                        AnimatorSet animatorSet = new AnimatorSet();
    ...
    protected class MyOnItemSelectedListener implements AdapterView.OnItemSelectedListener
    {
        ...
        public void onItemSelected(AdapterView<?> parent, View view, int position, long id)
        {
            try
            {
                ...
                // Log.d(TAG, "onItemSelected:" + vg.getWidth() + " " + newFocus.getMeasuredWidth() + " w=" + (rect.left
                // - rect.right));
